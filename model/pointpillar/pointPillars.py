...
from point_utils import get_paddings_indicator
from point_tools import change_default_args
...
class PFNLayer(nn.Module):
    def __init__(self,
        ...
        in_channels：输入通道的数量，表示输入特征的维度或通道数。
        out_channels：输出通道的数量，表示输出特征的维度或通道数。
        use_norm：一个布尔值，表示是否使用批量归一化（BatchNorm）。
        last_layer：一个布尔值，表示是否为最后一层。如果是最后一层，那么在该层中不会进行特征的连接（concatenation）。
        """
...
class PillarFeatureNet(nn.Module):
    def __init__(self,
        ...
        这段描述是关于 Pillar Feature Net 的说明。这个网络用于准备柱状体特征，并通过 PFN 层进行前向传播。这个网络的作用类似于 SECOND 中的 VoxelFeatureExtractor。
        num_input_features：输入特征的数量，可以是 x、y、z 或 x、y、z、r（颜色）的数量。
        use_norm：一个布尔值，表示是否使用批量归一化（BatchNorm）。
        num_filters：一个整数或整数列表 N，表示每个 PFN 层中的特征数量。
        with_distance：一个布尔值，表示是否包括点到点之间的欧几里得距离。
        voxel_size：一个浮点数或浮点数列表，表示体素（Voxel）的大小，仅利用 x 和 y 的大小。
        pc_range：一个浮点数或浮点数列表，表示点云的范围，仅利用 x 和 y 的最小值。
        """
...
class PointPillarsScatter(nn.Module):
    def __init__(self,
        ...
        这段描述是关于 Point Pillar 中的 Scatter 操作的说明。这个操作的作用是将从密集张量中学习到的特征转换为稀疏的伪图像。它替代了 SECOND 中的 SparseMiddleExtractor。
        output_shape：一个包含四个整数的列表，表示期望的输出特征的形状。
...
if __name__ == "__main__":
    ...
    # Step 1: Prepare data (假设数据已准备好)

    # Step 2: Build model
    pillar_feature_net = PillarFeatureNet(num_input_features=4, use_norm=True, num_filters=(64,))
    scatter = PointPillarsScatter(output_shape=[1, 64, 100, 100], num_input_features=64, batch_size=1)

    # Step 3: Forward pass
    pillar_x = ...  # 输入点云的 x 坐标
    pillar_y = ...  # 输入点云的 y 坐标
    pillar_z = ...  # 输入点云的 z 坐标
    pillar_i = ...  # 输入点云的 intensity（强度）
    num_voxels = ...  # 每个柱状体中的点的数量
    x_sub_shaped = ...  # x 的子网格
    y_sub_shaped = ...  # y 的子网格
    mask = ...  # 掩码，用于标记有效的柱状体

    pillar_feature = pillar_feature_net(pillar_x, pillar_y, pillar_z, pillar_i, num_voxels, x_sub_shaped, y_sub_shaped,
                                        mask)
    output = scatter(pillar_feature)
